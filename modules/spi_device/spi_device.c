...
bool spi_device_init(struct spi_device_s* dev, SPIDriver* spi_driver, uint32_t sel_line, uint32_t max_speed_hz, uint8_t data_size, uint8_t flags) {
    if (!dev) {
        return false;
    }

    dev->spi_driver = spi_driver;
...
void spi_device_begin(struct spi_device_s* dev) {
    ...
    spiAcquireBus(dev->spi_driver);
    dev->bus_acquired = true;

    spiStart(dev->spi_driver, &dev->spiconf);
...
void spi_device_send(struct spi_device_s* dev, uint32_t n, const void* txbuf) {
    ...
    if (dev->bus_acquired) {
        spiSend(dev->spi_driver, n, txbuf);
    } else {
        spi_device_begin(dev);
        spiSend(dev->spi_driver, n, txbuf);
        spi_device_end(dev);
...
void spi_device_receive(struct spi_device_s* dev, uint32_t n, void* txbuf) {
    ...
    if (dev->bus_acquired) {
        spiReceive(dev->spi_driver, n, txbuf);
    } else {
        spi_device_begin(dev);
        spiReceive(dev->spi_driver, n, txbuf);
        spi_device_end(dev);
...
void spi_device_exchange(struct spi_device_s* dev, uint32_t n, const void* txbuf, void* rxbuf) {
    ...
    if (dev->bus_acquired) {
        spiExchange(dev->spi_driver, n, txbuf, rxbuf);
    } else {
        spi_device_begin(dev);
        spiExchange(dev->spi_driver, n, txbuf, rxbuf);
        spi_device_end(dev);
