...
void setup_usb_strings(void)
{
    ...
    char serial[25];
    uint8_t cpu_id[12];
    board_get_unique_id(cpu_id, 12);
    for (uint8_t i=0; i<12; i++) {
        serial[i*2] = hex[(cpu_id[i]>>4)&0xF];
        serial[i*2+1] = hex[cpu_id[i]&0xF];
    }
    serial[24] = '\0';
    setup_usb_string(&vcom_strings[1], "Hex/ProfiCNC", vcom_buffers[0]);
    setup_usb_string(&vcom_strings[2], "SLCAN Adapter", vcom_buffers[1]);
    setup_usb_string(&vcom_strings[3], serial, vcom_buffers[2]);
}
